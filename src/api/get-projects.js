async function getProjects() {
    ...
    // method is set to `GET` because we are fetching data. Fetch returns a
    //"promise".
    ...
    // We can use the `ok` property on `response` to check if the request was
    //successful.
    ...
    // Turing the response to JSON takes time so we need to use the `await`
    //keyword again.
